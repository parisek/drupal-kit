...
- **Pattern (module-level hooks).** A hook is not subclassed and takes no arguments from the consumer, so neither pattern above fits. It reads `$this->featureFlags->enabled(FeatureFlags::FLAG_X)` instead — the `drupal_kit.feature_flags` service, injected into the `#[Hook]` class — a declared boolean in `drupal_kit.feature_flags`, the shape core uses for `system.feature_flags`. A flag ships five things in one commit: the schema key (`type: boolean`, a label in a site owner's words, `requiredKey: false`), the `false` default in `config/install`, a constant plus a `KNOWN_FLAGS` entry on `FeatureFlags` (a constant, not a constructor argument: schema validation runs in tests, never on a production read, and the other two patterns get the same guarantee free from PHP — a misspelled property or argument does not compile, a misspelled config key is silence), a `post_update` that sets the key FALSE where it is absent so existing sites see it in a config diff, and both branches in the kernel test. Unknown, absent and malformed all read FALSE: a site whose config predates the flag behaves exactly as it did before the upgrade (#115).
