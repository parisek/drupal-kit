...
- **Pattern (module-level hooks).** A hook is not subclassed and takes no arguments from the consumer, so neither pattern above fits. It reads `FeatureFlags::enabled('<flag>')` instead — one boolean in the `features` map of `drupal_kit.settings`, absent and therefore off until a project sets it. Document the flag name beside the behaviour it gates, and cover both branches in the kernel test. Unknown, absent and malformed all read FALSE: a site whose config predates the flag must behave exactly as it did before the upgrade (#115).
