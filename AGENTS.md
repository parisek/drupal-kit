...
- **Pattern (module-level hooks).** A hook is not subclassed and takes no arguments from the consumer, so neither pattern above fits. It reads `$this->featureFlags->enabled(FeatureFlags::FLAG_X)` instead: a declared boolean in the `drupal_kit.feature_flags` config object, the shape core uses for `system.feature_flags`. The reader is the `drupal_kit.feature_flags` service, injected into the `#[Hook]` class with `#[Autowire(service: 'drupal_kit.feature_flags')]` — the same way `Libraries` takes `vite_manifest`. **The attribute is not optional**: the service id is not the class name, so a bare `FeatureFlags $featureFlags` type-hint fails the container build. A flag ships five things in one commit: the schema key (`type: boolean`, a label in a site owner's words, `requiredKey: false`), the `false` default in `config/install`, a constant plus a `KNOWN_FLAGS` entry on `FeatureFlags` (schema validation runs in tests and Config Inspector, never on a production read — the allowlist is the runtime guard. It is a constant rather than a constructor argument because it is module-owned data, not wiring; it is **not** harder to override that way, since a ServiceProvider can swap the class either way), a `post_update` that sets the key FALSE where it is absent so existing sites see it in a config diff, and both branches in the kernel test. Unknown, absent and malformed all read FALSE: a site whose config predates the flag behaves exactly as it did before the upgrade (#115).
