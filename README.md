...
- `Drupal\drupal_kit\Services\FeatureFlags` — static utility: reads the opt-in flags in `drupal_kit.settings`. `FeatureFlags::enabled('<flag>')` is FALSE until a project sets it, which is how hook-level behaviour ships opt-in.
