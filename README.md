...
- `Drupal\drupal_kit\Services\FeatureFlags` — static utility: reads the opt-in flags in `drupal_kit.feature_flags`, the shape core uses for `system.feature_flags`. `FeatureFlags::enabled('<flag>')` is FALSE unless the module declares the flag **and** the project set it, which is how hook-level behaviour ships opt-in.
