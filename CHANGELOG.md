...
### Added
- **`MenuActiveTrailResolver` @covers annotations for private helpers** — four existing unit tests (`testFallbackUsesDeepestBreadcrumbMatch`, `testTrailIncludesMenuParents`, `testPrefersShallowestMatchAmongDuplicates`, `testSkipsUnroutedBreadcrumbCrumbs`) exercise `pickShallowest` + `buildTrailFromLink` transitively from `getActiveTrailIds`, but PHPUnit's strict `@covers ::publicMethod` filter only credits lines inside the listed method — transitively-called private helpers stay uncounted even though they run (a `die()` injection confirmed they execute). Adding `@covers ::pickShallowest` + `@covers ::buildTrailFromLink` on those four tests credits the lines that were always being executed. `MenuActiveTrailResolver` coverage jumps 52 % → 98 % (39/40 lines) with zero new tests written — the bodies were already exercised; only the metric was lying.
